...
export default function DashboardRightPanel({ tr, todos, notifications, dueItems }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              {t.href ? (
                <button
                  ...
                  onClick={() => router.push(t.href!)}
                    className="flex w-full items-start gap-2 rounded-lg border border-border bg-surface-2/60 px-3 py-2 text-left transition hover:border-signal/40"
                >
                  <span className="mt-0.5 h-4 w-4 shrink-0 rounded border border-signal/50" />
      ...
      {dueItems.length > 0 && (
        <div className="data-panel border-signal/30 bg-signal/5 p-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-signal">{tr('Due soon', "Tez orada to'lash")}</h3>
              ...
              <li key={d.id}>
                {d.href ? (
                  <button
                    ...
                    className="w-full text-left text-xs transition hover:text-signal"
                  >
      ...
      <div className="data-panel p-4">
        ...
        <Link href="/reports" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-signal hover:underline">
