...
async function main() {
    ...
    const firmA = await prisma.firm.create({
        data: {
            ...
            currency: 'USD',
            status: client_1.FirmStatus.ACTIVE,
            users: {
                create: [{ email: 'alpha@firm.com', password: HASHED_PASSWORD_1111, role: client_1.Role.FIRM }]
            }
        }
    });
    const firmB = await prisma.firm.create({
        data: {
            ...
            status: client_1.FirmStatus.ACTIVE,
            users: {
                create: [{ email: 'beta@firm.com', password: HASHED_PASSWORD_1111, role: client_1.Role.FIRM }]
            }
        }
    });
    const firmC = await prisma.firm.create({
        data: {
            ...
            currency: 'USD',
            status: client_1.FirmStatus.SUSPENDED,
            users: {
                create: [{ email: 'gamma@firm.com', password: HASHED_PASSWORD_1111, role: client_1.Role.FIRM }]
            }
        }
    });
    ...
    const flightOpen = await prisma.flight.create({
        data: {
            ...
            currency: 'USD',
            settlementStatus: client_1.FlightSettlementStatus.OPEN,
        }
    });
    const flightClosing = await prisma.flight.create({
        data: {
            ...
            currency: 'USD',
            settlementStatus: client_1.FlightSettlementStatus.CLOSING,
        }
    });
    const flightClosed = await prisma.flight.create({
        data: {
            ...
            currency: 'USD',
            settlementStatus: client_1.FlightSettlementStatus.CLOSED,
        }
    });
    // 5. Generate Tickets (10 for each flight)
    const basePrice = 500.00;
    const ticketsOpen = await Promise.all(Array.from({ length: 10 }).map(() => prisma.ticket.create({
        data: { flightId: flightOpen.id, basePrice, currency: 'USD' }
    })));
    const ticketsClosing = await Promise.all(Array.from({ length: 10 }).map(() => prisma.ticket.create({
        data: { flightId: flightClosing.id, basePrice, currency: 'USD' }
    })));
    const ticketsClosed = await Promise.all(Array.from({ length: 10 }).map(() => prisma.ticket.create({
    ...
    const allocationTx1 = await prisma.transaction.create({
        data: {
            type: client_1.TransactionType.ALLOCATION,
            firmId: firmA.id,
    ...
    for (const t of ticketsOpen.slice(0, 4)) {
        await prisma.ticket.update({
            where: { id: t.id },
            data: { status: client_1.TicketStatus.ALLOCATED, assignedFirmId: firmA.id }
        });
    }
    ...
    for (const t of soldTickets1) {
        await prisma.ticket.update({
            where: { id: t.id },
            data: { status: client_1.TicketStatus.SOLD, soldPrice: sellPrice1, soldCurrency: 'USD', purchaserInfo: { name: 'John Doe' } }
        });
    }
    // -----------------------
    ...
    for (const t of allocTicketsB) {
        await prisma.ticket.update({ where: { id: t.id }, data: { status: client_1.TicketStatus.ALLOCATED, assignedFirmId: firmB.id } });
    }
    ...
    await prisma.ticket.update({
        ...
        data: { status: client_1.TicketStatus.SOLD, soldPrice: sellPriceEur, soldCurrency: 'EUR' }
    });
    ...
    await prisma.ticket.update({
        where: { id: allocTicketsB[0].id },
        data: { status: client_1.TicketStatus.REFUNDED }
    });
    // Make a Payment
    const paymentAmt = 2000.00;
    const payment = await prisma.payment.create({
        data: {
            firmId: firmB.id, amount: paymentAmt, method: client_1.PaymentMethod.BANK_TRANSFER,
            reference: 'WIRE-12345', status: client_1.PaymentStatus.CONFIRMED, reconciliationStatus: client_1.ReconciliationStatus.MATCHED
        }
    });
    await prisma.transaction.create({
        data: {
    ...
    await prisma.ticket.update({ where: { id: ticketsClosed[0].id }, data: { status: client_1.TicketStatus.ALLOCATED, assignedFirmId: firmA.id } });
    await prisma.ticket.update({ where: { id: ticketsClosed[1].id }, data: { status: client_1.TicketStatus.SOLD, assignedFirmId: firmA.id, soldPrice: 800.0, soldCurrency: 'USD' } });
    await prisma.transaction.create({
        data: {
